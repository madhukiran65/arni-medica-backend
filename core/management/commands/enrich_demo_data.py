...
class Command(BaseCommand):
    ...
    def enrich_change_controls(self):
        ...
        change_controls_data = [
            {
                "change_control_id": "CC-2026-0001",
            ...
            {
                "change_control_id": "CC-2026-0002",
            ...
            {
                "change_control_id": "CC-2026-0003",
            ...
            {
                "change_control_id": "CC-2026-0004",
        ...
        for cc_data in change_controls_data:
            try:
                change_control = ChangeControl.objects.get(change_control_id=cc_data["change_control_id"])
                for key, value in cc_data.items():
                    if key != "change_control_id":
                        setattr(change_control, key, value)
                change_control.save()
                self.stdout.write(
                    self.style.SUCCESS(f"  ✓ Updated {cc_data['change_control_id']}")
                )
            except ChangeControl.DoesNotExist:
                self.stdout.write(
                    self.style.WARNING(
                        f"  ⚠ ChangeControl {cc_data['change_control_id']} not found, skipping"
                    )
                )
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(
                        f"  ✗ Error updating {cc_data['change_control_id']}: {str(e)}"
                    )
                )
        ...
        suppliers_data = [
            {
                ...
                "iso_expiry_date": today + timedelta(days=365),
                "risk_justification": "Medium risk due to single-source for 2 critical reagents",
                "qualified_date": today - timedelta(days=180),
                "next_evaluation_date": today + timedelta(days=185),
                "products_services": [
            ...
            },
            {
                ...
                "iso_expiry_date": today + timedelta(days=400),
                "risk_justification": "Medium risk - alternative suppliers available but qualification required",
                "qualified_date": today - timedelta(days=270),
                "next_evaluation_date": today + timedelta(days=95),
                "products_services": [
                    "Plastic vials",
            ...
            },
            {
                ...
                "iso_expiry_date": today + timedelta(days=350),
                "risk_justification": "Low risk - multiple alternative providers available, non-critical path",
                "qualified_date": today - timedelta(days=200),
                "next_evaluation_date": today + timedelta(days=165),
                "products_services": [
            ...
            },
            {
                ...
                "iso_expiry_date": today + timedelta(days=300),
                "risk_justification": "High risk - sole source for proprietary antibodies, long lead times",
                "qualified_date": today - timedelta(days=365),
                "next_evaluation_date": today + timedelta(days=90),
                "products_services": [
            ...
            },
            {
                ...
                "next_evaluation_date": today + timedelta(days=60),
                "products_services": [
